...
def run_training_loop(args):
    ...
    for itr in range(args.n_iter):
        print(f"\n********** Iteration {itr} ************")
        # ODO: sample `args.batch_size` transitions using utils.sample_trajectories
        # make sure to use `max_ep_len`
        trajs, envsteps_this_batch = utils.sample_trajectories(env, agent.actor, min_timesteps_per_batch=args.batch_size, max_length=max_ep_len)
        ...
        # ODO: train the agent using the sampled trajectories and the agent's update function
